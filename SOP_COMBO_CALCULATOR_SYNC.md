...
这里的“兼容”指常规数据和计算规则更新可以继续使用 Haven 自有 Angular 展示层，
不表示无条件接受任意上游结构变化。

| 上游变化 | 预期结果 |
|---|---|
| 现有五个数据集内部的数值或条目变化 | 两种模式仍是合法 JSON 时自动同步。 |
| 既有领域函数边界内的计算规则变化 | Angular 适配仍能应用且完整发布门禁通过时同步。 |
| 上游 UI 框架、HTML 或样式变化 | 不进入生产产物；只有同时破坏数据提取边界时才影响同步。 |
| 数据集名称/数量、提取标记、函数签名或浏览器依赖变化 | 同步或生成必须 fail-closed；显式更新适配器和回归测试。 |
| 部署脚本与 GitHub commit 不一致，或同步期间上游内容发生变化 | 停止同步，禁止生成混合来源快照。 |

结构性不兼容不得通过发布上游 HTML、恢复 jQuery/Bootstrap/Vue 或增加兼容层绕过。
