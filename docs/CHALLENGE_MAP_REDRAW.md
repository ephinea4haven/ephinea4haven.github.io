...
| Page legends | EP1 uses 12 entries drawn in the map symbols' own style. EP2 uses 14: route lines, terminals, a callout swatch and seven mechanism icons (paired warp letters, matching-colour switches, timed and light switches, healing, barriers/traps, supply boxes). The mechanism icons are crops of the traced Ephinea Wiki artwork (`legend` in `content/challenge-maps/ep2.json` names a sample area and source box); `scripts/build_ep2_challenge_map_atlas.py` writes them to `assets/img/challenge/ep2/legend/` for the page and draws the same crops in each map's legend panel, with wording from the shared `challenge.legend*` messages. |
