...
后续命名策略讨论未修改 `Caduceus → 赫尔墨斯杖`、`Excalibur → 王者之剑`、`Glide Divine → 澄蓝之杖「神游」`。本站继续使用权威译名；同步审查通过不等于重新审定每个译名，也不替代客户端游戏内显示验证。文档、代码及生成数据随本次 master 提交交接，线上 Pages 发布结果另行核对。
