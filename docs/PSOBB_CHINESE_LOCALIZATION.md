...
```text
Photon Drop    → 光子微晶ＰＤ
Photon Crystal → 光子水晶ＰＣ
Photon Sphere  → 光子结晶ＰＳ
```
...
| `Technique disk` / ` disk`（学习魔法的动态物品名） | 魔法光盘 / `【光盘】` | 类别写“魔法光盘”；具体物品使用掉落表中的魔法名并接客户端后缀 `【光盘】`，如“火球术【光盘】”。不得写成“法术盘”“技能盘”或“魔法书”。 |
| `Grinder`（提升武器磨数的道具或数值） | 打磨石 / 磨数 | 道具名使用“小打磨石 / 中打磨石 / 大打磨石”；武器的 `+N` 数值按语境写“磨数”，不能笼统译成制作材料。 |
| `Photon Drop` / `Photon Crystal` / `Photon Sphere` | 光子微晶ＰＤ / 光子水晶ＰＣ / 光子结晶ＰＳ | 三者是不同物品，名称必须同时与中文客户端和掉落表权威词典对齐；禁止按字面写成“光子水滴”，也不得混用“水晶”“结晶”或缩写。 |
...
- `Technique disks` 作为物品类别 → **魔法光盘**；具体名称使用客户端动态后缀 **`【光盘】`**。
