...
export class ItemCatalogComponent {
  ...
  readonly subtypes = computed(() => [...new Set(ITEMS.filter(i => i.category === this.category()).map(i => i.subtype))]);
  ...
  readonly listParams = computed<Params>(() => ({ lang: this.i18n.language(), q: this.query() || null, category: this.category(), type: this.subtype() || null, status: this.status() || null, class: this.profession() || null, rarity: this.rarity() || null, images: this.imagesOnly() ? '1' : null, sort: this.sort() === 'catalog' ? null : this.sort(), page: this.page() === 1 ? null : this.page() }));
  ...
  count(category: string): number { return ITEMS.filter((item) => item.category === category).length; }
  ...
  clear(): void { void this.router.navigate([], { relativeTo: this.route, queryParams: {lang:this.i18n.language(),category:this.category()}, replaceUrl: true }); }
