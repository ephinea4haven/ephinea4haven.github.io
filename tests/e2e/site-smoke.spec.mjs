...
test('anniversary archive defaults to the announced 2026 event and keeps 2025 available', async ({ page }) => {
  ...
  await expect(page.locator('#anniv-2026-milestones')).toContainText('服务器点数为 4,462');
  ...
  expect(await page.locator('#anniv-2026-milestones .shop-table').nth(1).locator('tbody tr').allTextContents())
    .toEqual([
      'Forest22,707', 'Cave4,469', 'Mine5,390', 'Ruins4,475',
      'Temple4,471', 'Spaceship4,469', 'CCA6,879', 'Seabed4,468',
      'Tower4,465', 'Crater4,462', 'Desert10,764',
    ]);
...
test('anniversary years expose complete localized milestone contracts', async ({ page }) => {
  const expected = [
    { year: 2026, total: '服务器点数为 4,462', rows: 16, first: ['1,000', '稀有物品掉落率 +10%'], last: ['20,000', '官方暂未公开'] },
