...
test('anniversary archive defaults to the announced 2026 event and keeps 2025 available', async ({ page }) => {
  ...
  const questPointRows = await page.locator('#anniv-2026-milestones .shop-table').nth(1).locator('tbody tr').evaluateAll(
    (rows) => rows.map((row) => ({
      ...
      points: Number.parseInt(row.cells[1].textContent.replaceAll(',', ''), 10),
    })),
  );
  expect(questPointRows.map(({ name }) => name)).toEqual([
    'Forest', 'Cave', 'Mine', 'Ruins', 'Temple', 'Spaceship',
    'CCA', 'Seabed', 'Tower', 'Crater', 'Desert',
  ]);
  expect(questPointRows.every(({ points }) => Number.isInteger(points))).toBe(true);
  expect(serverPoints).toBe(Math.min(...questPointRows.map(({ points }) => points)));
...
test('Angular price guide filters categories and bilingual item names', async ({ page }) => {
  ...
  await expect(page.locator('.price-notice')).toContainText('PD（Photon Drop / 光子微晶）');
